...
function App() {
  ...
  return <>
    ...
    <header><nav className="container nav"><a href="#" className="brand"><Mark/>IntentGraph<span className="beta">BETA</span></a><div className={`nav-links ${menu ? 'open' : ''}`}>{[['The protocol','#protocol'],['Live activity','#activity'],['Developers','#developers'],['Playground','/playground']].map(([text,href]) => <a key={href} href={href} onClick={()=>setMenu(false)}>{text}</a>)}</div><a className="button small nav-cta" href="#developers">Connect your agent <ArrowUpRight size={15}/></a><button className="menu" aria-label="Toggle navigation" aria-expanded={menu} onClick={()=>setMenu(!menu)}>{menu ? <X/> : <Menu/>}</button></nav></header>
    <main id="main">
      <section className="hero container">
        <div className="hero-copy"><div className="eyebrow"><span className="line"/> ON-CHAIN DATA. AGENT-READY.</div><h1>From intent<br/>to <em>on-chain</em><br/>intelligence.</h1><p>Your agent brings the question. We open the data.<br className="desktop"/> Discover and query The Graph’s subgraphs with a single MCP connection and a small HBAR payment.</p><div className="hero-actions"><a className="button" href="#developers">Connect your agent <ArrowUpRight size={18}/></a><a className="text-link" href="/playground">Try the live demo <ArrowRight size={16}/></a></div><div className="hero-note"><Check size={14}/> No Graph API key needed <span>·</span> Pay with HBAR</div></div>
