import PackageCards, { type AccessPackage } from './PackageCards';
...
function App() {
  ...
  return <>
    ...
    <header><nav className="container nav"><a href="#" className="brand"><Mark/>IntentGraph<span className="beta">BETA</span></a><div className={`nav-links ${menu ? 'open' : ''}`}>{[['The protocol','#protocol'],['Live activity','#activity'],['Developers','#developers'],['Playground','/playground']].map(([text,href]) => <a key={href} href={href} className={href === '/playground' ? 'playground-link playground-nav' : undefined} onClick={()=>setMenu(false)}>{href === '/playground' && <span className="playground-dot" aria-hidden="true"/>}{text}{href === '/playground' && <ArrowUpRight size={13} aria-hidden="true"/>}</a>)}</div><a className="button small nav-cta" href="#developers">Connect your agent <ArrowUpRight size={15}/></a><button className="menu" aria-label="Toggle navigation" aria-expanded={menu} onClick={()=>setMenu(!menu)}>{menu ? <X/> : <Menu/>}</button></nav></header>
    <main id="main">
      <section className="hero container">
        <div className="hero-copy"><div className="eyebrow"><span className="line"/> ON-CHAIN DATA. AGENT-READY.</div><h1>From intent<br/>to <em>on-chain</em><br/>intelligence.</h1><p>Your agent brings the question. We open the data.<br className="desktop"/> Discover and query The Graph’s subgraphs with a single MCP connection and a small HBAR payment.</p><div className="hero-actions"><a className="button" href="#developers">Connect your agent <ArrowUpRight size={18}/></a><a className="playground-link playground-hero" href="/playground"><span className="playground-dot" aria-hidden="true"/>Try the playground <ArrowRight size={16} aria-hidden="true"/></a></div><div className="hero-note"><Check size={14}/> No Graph API key needed <span>·</span> Agent-selected HBAR packages</div></div>
      ...
      <section className="pricing container package-pricing"><div><div className="eyebrow">TIME-BASED HBAR PACKAGES</div><h2>A quick lookup.<br/><em>Or room to explore.</em></h2><p className="package-intro">Pay for the access window you need. In the playground, the agent chooses the cheapest suitable package for your intent.</p></div><PackageCards packages={stats?.packages}/><div className="price-content"><a href="/playground" className="button">Let your agent choose <ArrowUpRight size={17}/></a><small>Quote validity is the time available to submit payment. Your access window starts after settlement. Prices are locked per quote; no subscription or automatic renewal. Testnet HBAR only. Usage limits and session closure also end access. No automatic refunds.</small></div></section>
