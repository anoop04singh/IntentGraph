...
export class DemoWallet {
  ...
  async status(force = false): Promise<WalletStatus> {
    ...
    const ready = !data.deleted && Boolean(data.key) && balance >= BigInt(accessPackages(this.config)[0].amount);
  ...
  async sign(requirements: PaymentRequirements) {
    if (requirements.scheme !== 'exact' || requirements.network !== 'hedera:testnet' || requirements.asset !== '0.0.0' ||
      requirements.payTo !== this.config.HEDERA_SELLER_ACCOUNT_ID || !accessPackages(this.config).some(p => p.amount === requirements.amount) || requirements.maxTimeoutSeconds > 120) throw new Error('The demo wallet rejected unexpected payment requirements.');
    const status = await this.status(true);
    if (!status.ready || !status.accountId) throw new Error(status.message);
    if (BigInt(status.balanceTinybars) < BigInt(requirements.amount)) throw new Error('The shared wallet needs more testnet HBAR for the selected package.');
